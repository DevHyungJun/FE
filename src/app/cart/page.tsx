...
export default function Cart() {
  ...
  const handleAllCheckClick = () => {
    if (selectedItems.length === cartData?.article_list.length) {
      ...
      setSelectedItems(cartData?.article_list.map((item: any) => item.article) || []);
    }
  };
  ...
  const handleOrder = () => {
    ...
    const orderData = selectedItems.map((item) => ({
      product: item,
      quantity: cartData?.article_list.find((articleItem: { article: string }) => articleItem.article === item)?.quantity
    }));
          ...
          {cartData?.article_list.length !== 0 ? (
            <>
              ...
              <div className="flex justify-between">
                <Checkbox
                  isSelected={selectedItems.length === cartData?.article_list.length}
              ...
              <CheckboxGroup value={selectedItems}>
                {cartData?.article_list?.map((articleItem: { article: string; quantity: number; _id: string }) => (
                  <div className="flex" key={articleItem?.article}>
                    <Checkbox value={articleItem?.article} isSelected={selectedItems.includes(articleItem.article)} onChange={() => handleItemCheck(articleItem?.article)} />
                    <CartProductDetail articleId={articleItem?.article} quantity={articleItem?.quantity} setPrices={setPrices} />
                  </div>
                ))
                }
              </CheckboxGroup>
              <Button color="primary"
                ...
                onClick={handleOrder}
              >
                <p>{totalPrice !== 0 && formatPrice(totalPrice)}{totalPrice !== 0 ? ' 결제하기' : '상품을 선택해주세요'}</p>
              </Button>
            </>
          ) : (
            <div className="flex flex-col items-center gap-3 mt-10">
              <h2 className="text-lg font-semibold m-1">장바구니에 담긴 상품이 없습니다.</h2>
              <p className="text-gray-700 text-sm">상품을 추가해보세요.</p>
              <button className="text-sm border p-1 rounded-md mt-3" onClick={()=>setCartOrFavorite('favorite')}>좋아요한 상품 보기</button>
