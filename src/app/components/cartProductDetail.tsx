...
export default function CartProductDetail({ articleId, setPrices, quantity }: CartProductDetailProps) {
  const { data: productData, isSuccess: productIsSuccess, isPending } = useDetail(articleId, !!articleId);
  ...
  useEffect(() => {
    setPrices((prev: any) => {
      if (productIsSuccess) {
        ...
        const updatedPrices = prev.filter((item: { id: string; }) => item.id !== articleId); // 동일 ID 항목 제거
    ...
    <div className="flex-grow border-b p-3 rounded-sm">
      <div className="flex gap-3">
        ...
        <div className="flex-grow space-y-2 text-sm">
          {isPending ? <LoadingSpinner mode="1" /> : (
            <>
              ...
              <p className="text-gray-500">{formatPrice(productData?.data?.product?.price)} / {quantity}개</p>
