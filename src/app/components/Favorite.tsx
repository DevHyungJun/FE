...
export default function Favorite({
  ...
  const handleAddCart = (id: string) => {
    const quantityValue = quantity?.find(
      (item: any) => item.id === id
    )?.quantity;
      ...
      {likeList?.length !== 0 ? (
        <>
          ...
          {likeList?.map((likeItem: PostData) => (
            <div
              className="flex-grow border-b p-3 rounded-sm"
              key={likeItem?._id}
            >
                ...
                <div className="flex-grow space-y-2 text-sm">
                  <>
                    <div className="flex justify-between text-sm">
                      <Link
                        href={`/products/product-detail/${likeItem?._id}`}
                        className="hover:font-semibold"
                      >
                        <p className="text-xs md:text-sm">
                          {likeItem?.product?.product_name}
                        </p>
                      </Link>
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          onClick={() => handleDelteFavorite(likeItem?._id)}
                        >
                          <IoIosClose />
                        </button>
                      </div>
